cCCA <- function(otutab, env, mySite=NULL,scale=FALSE,choices=1:2,display = "sites",
  ...
  if(scale) obj <- cca(otutab, env, scale = TRUE)
  else obj <- cca(otutab, env)
  xlbl = paste0('CCA1 ', getpropX(obj))
  ylbl = paste0('CCA2 ', getpropY(obj))
  ...
  plot(obj, main=paste0("Constrained CCA plot for ",deparse(substitute(otutab))),
       cex.lab = 1.5, cex.axis=1.2,xlab = xlbl, ylab = ylbl)
