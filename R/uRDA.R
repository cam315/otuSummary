uRDA <- function(otutab,env = NULL, mySite=NULL, scale=FALSE, biplot=FALSE, choices=1:2, display = "sites", showsp=TRUE, 
  ...
  print(summary(obj))
  xlbl = paste0('PC1 ', getpropX(obj))
  ylbl = paste0('PC2 ', getpropY(obj))
  ...
  legends= TRUE}
  if(biplot){
    biplot(obj, scaling = 3, type = c("text", "points"), cex.lab=1.5,cex.axis=1.2,
    ...
    if(showsp){
      points(obj, display="sp", cex=0.8, pch= '+', col='red')
